...
unsigned int hash_pjw(void *key) {
    ...
    for (hash_value = 0; *datum; ++datum) {
        ...
        if ((i = hash_value & HIGH_BITS) != 0) hash_value = (hash_value ^ (i >> THREE_QUARTERS)) & ~HIGH_BITS;
    }
    return (hash_value);
}
...
void *icl_hash_find(icl_hash_t *ht, void *key) {
    ...
    for (curr = ht->buckets[hash_val]; curr != NULL; curr = curr->next)
        if (ht->hash_key_compare(curr->key, key)) return (curr->data);
...
icl_entry_t *icl_hash_insert(icl_hash_t *ht, void *key, void *data) {
    ...
    for (curr = ht->buckets[hash_val]; curr != NULL; curr = curr->next)
        if (ht->hash_key_compare(curr->key, key)) return (NULL); /* key already exists */
...
icl_entry_t *icl_hash_update_insert(icl_hash_t *ht, void *key, void *data, void **olddata) {
    ...
    if (olddata != NULL && *olddata != NULL) *olddata = NULL;
...
int icl_hash_dump(FILE *stream, icl_hash_t *ht) {
    ...
    for (i = 0; i < ht->nbuckets; i++) {
        bucket = ht->buckets[i];
        for (curr = bucket; curr != NULL;) {
            if (curr->key) fprintf(stream, "icl_hash_dump: %s: %p\n", (char *)curr->key, curr->data);
            curr = curr->next;
        }
    }
